        ...
        if e.build_log:
            for log_entry in e.build_log:
                if "stream" in log_entry:
                    ...
                    print(
                        Fore.RED + log_entry["errorDetail"]["message"] + Style.RESET_ALL
                    )
...
if not os.path.exists(downloaded_resources_path):
    os.makedirs(downloaded_resources_path)
resources_index_path = os.path.join(downloaded_resources_path, "index.json")
...
for resource_url, requiring_recipes in resources_to_download_for_recipes.items():
    if resource_url not in resources_index:
        ...
        resource_path = os.path.join(downloaded_resources_path, resource_file_name)
        try:
...
while not recipes_to_run.empty():
    ...
    for resource_alias, resource_url in recipe["required_resources"][
        ...
        volumes[
            os.path.join(downloaded_resources_path, resources_index[resource_url])
        ] = {
